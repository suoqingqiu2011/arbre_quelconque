...
void printArray(int *a, int len,int (*tab)[2],int **T,int hauteur,int *H,char **strings){
    int i,j;
    int s1;
    int haut=0;

    int *cpt;
    cpt=malloc(sizeof(int*)*hauteur+1);
/*
...
void combine(int *Array, int nb_array,int apos,int *btmp,int nbarc,int bpos,int (*tab)[2],int **T,int hauteur,int *H,char **strings)
...
void create_tree(int *H,Hauteur *haut_t,int n_total,char **strings){
/*int main(){
   int *H;
    H=malloc(sizeof(int*));
     H[0]=1;
...
/*
   ...
   // printf("D->nb_compteur: %d\n",D.nb_compteur);
    ...
    int * btmp=(int*)malloc(sizeof(int)*nb_arc+1);
    
	int *Array;
    Array=(int *) malloc(sizeof(int)*nb_array+1);
